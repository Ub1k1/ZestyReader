...
# Mark the selected text for paste later
def copy_text():
...
def paste_text():
    ...
    # If we do not allow paste multiple time, we need clean the clipboard
    # root.clipboard_clear()


# Use build in Text's own function edit_undo to undo the change
def undo_text():
    text.edit_undo()


# Use build in Text's own function edit_redo to redo the change
def redo_text():
    text.edit_redo()


# Search the text and highlight the word
def find_text():
    ...
    # Action button to do the search or cancel.
    # If you need pass the parameter to the function, must use 'lambda:' keyword
...
# Search the text from the beginning to the end. If the word found, highlight it with yellow
...
def italic_text():
    ...
    if text.tag_ranges("sel"):
        ...
        tags = text.tag_names(SEL_FIRST)
        tag_found = 0
        # Check ig the selected text has a italic tag
        for tag_name in tags:
...
def underline_text():
    ...
    if text.tag_ranges("sel"):
        ...
        tags = text.tag_names(SEL_FIRST)
        tag_found = 0
        # Check ig the selected text has a underline tag
        for tag_name in tags:
